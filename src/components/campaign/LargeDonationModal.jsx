...
import LargeDonationAuth from './LargeDonationAuth';
...
const VIEW_ORDER = [
  ...
  'offline-form-a',
  'offline-form-b',
  'auth',            // ← inline auth step (only shown when user is not logged in)
  'offline-success',
];
...
export default function LargeDonationModal({
  ...
  /**
   * doSubmit — called either directly (user is logged in) or after
   * LargeDonationAuth succeeds (user just authenticated inline).
   */
  const doSubmit = async (authenticatedUser) => {
    // Resolve the user: prefer freshly-authenticated user, then Redux store
    const resolvedUser = authenticatedUser || userInfo;

    if (!resolvedUser) {
      // Not logged in → show inline auth step
      nav('auth');
      return;
    }
    ...
    try {
      const data = await createDonation({
        ...
        donorName:  resolvedUser.fullName || donorName  || '',
        email:      resolvedUser.email    || donorEmail || '',
        phone:      resolvedUser.mobileNo || donorPhone || '',
  ...
  // After inline auth succeeds, immediately submit
  ...
  const hdrMap = {
    ...
    'auth':            { title: 'Verify Your Identity',   sub: 'Sign in to confirm your transfer' },
  ...
  /* ── progress (exclude auth + success from bar) ─────────── */
  const progressSteps = VIEW_ORDER.filter(v => v !== 'offline-success' && v !== 'auth');
  const progressIdx   = progressSteps.indexOf(view);
  const progressPct   = view === 'offline-success' ? 100
    : view === 'auth' ? 100          // auth is the last "real" step before success
    : progressIdx < 0 ? 0
              ...
              <div className="flex-1 overflow-y-auto min-h-0" style={{ WebkitOverflowScrolling: 'touch' }}>
                <AnimatePresence mode="popLayout" custom={isForward}>
                  <motion.div key={view} custom={isForward}
                    ...
                    {/* ══════════════════════════════════════
                        AUTH STEP — inline, no separate modal
                    ══════════════════════════════════════ */}
                    {view === 'auth' && (
                      <LargeDonationAuth
                        dark={dk}
