...
useEffect(() => {
  async function fetchHijri() {
    try {
      ...
      // Parse gregorian date manually (API gives "dd-mm-yyyy")
      const [dayStr, monthStr, yearStr] = greg.date.split('-');
      const dayNum = parseInt(dayStr);
      const monthNum = parseInt(monthStr);
      const yearNum = parseInt(yearStr);

      // Convert to readable format (e.g., 25th October 2025)
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
          ...
          <div>
            ...
            <ul className="space-y-1.5 text-sm mb-6">
              {['About Us', 'FAQs', 'Legal & Help', 'Contact Us'].map(item => (
                <li key={item}>
